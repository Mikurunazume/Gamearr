...
export interface ImportStrategy {
   ...
   */
  planImport(sourcePath: string, game: Game, targetRoot: string, config: ImportConfig): Promise<ImportReview>;
  /**
...
export class PCImportStrategy implements ImportStrategy {
  canHandle(game: Game): boolean {
    ...
    return !!game.platforms?.includes("PC (Microsoft Windows)") || !!game.platforms?.includes("Win");
  }

  async planImport(sourcePath: string, game: Game, targetRoot: string, _config: ImportConfig): Promise<ImportReview> {
...
export class RomMImportStrategy implements ImportStrategy {
  ...
  async planImport(sourcePath: string, game: Game, targetRoot: string, config: ImportConfig): Promise<ImportReview> {
    ...
    if ((await fs.stat(sourcePath)).isDirectory()) {
      ...
      const candidates = files.filter(f => !config.ignoredExtensions.includes(path.extname(f).toLowerCase()));
    ...
    let filename = config.renamePattern
      .replace("{Title}", game.title)
      .replace("{Region}", "World"); // Placeholder for region, hard to extract from metadata without Indexer info.
